...
export function renderMasteryPanel(container, slot, mastery, state, over, cb, versionName, data) {
    ...
    const dedupedSkills = mastery.skills.filter(s => {
        if (!cellPositions.has(s.id))
            return false;
        return !Array.from(cellPositions.keys()).some(id => id === s.id && mastery.skills.indexOf(s) !== mastery.skills.findIndex(sk => sk.id === id));
    });
...
function renderSkillCell(skill, slot, state, over, cb, versionName, data) {
    ...
    if (!isTouch) {
        cell.addEventListener('click', (e) => {
            if (e.target.closest('.skill-controls'))
                return;
            e.preventDefault();
            if (unlocked && rank < skill.maxRank && !over)
                cb.onSkillDelta(skill.id, slot, 1);
        });
